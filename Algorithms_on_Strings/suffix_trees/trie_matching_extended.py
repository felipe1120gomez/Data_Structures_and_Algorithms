...
def build_trie(pat_list):
    ...
    for pattern in pat_list:
        pattern += '$' # mark indicating the end of the pattern
        node = root
...
def prefix_matching(cur_text, trie):
    '''Loops through the given portion of text and returns whether there is a pattern
    in it or not. Is able to handle cases when one of the patterns is a prefix of another
    pattern. In this case, some patterns are spelled in a trie by traversing a path from
    the root to an internal vertex, but not to a leaf.'''
    ...
    while True:
        ...
        if '$' in trie[node]: # if it is the end of the pattern
            return True
